...
@endif
<div class="container-fluid button-btn containerQuiz" >
...
<div class="container containerQuiz" >
   <div class="card cardAppointment">
    <div class="card-header cardAppointmentheader">
       <h3 class="tittle_form">Add question for {{$quiz->class_teacher->subject->name}} class {{$quiz->class_teacher->class->name}}
       on date {{$quiz->date}}: </h3>
    </div>
    <div class="card-body">
        <form  class="needs-validation" method="POST" action="{{route('add.question.for.quiz',$quiz->id)}}" enctype="multipart/form-data"  autocomplete="off" novalidate >
            @csrf
            <div class="row mb-3" >
                <label for="class" class="col-md-4 col-form-label text-md-end">{{ __('Quistion:') }}</label>
                <div class="col-md-4">
                   <textarea name="question" id="question" cols="35" rows="2" required></textarea>
                </div>
            </div>
            <div class="row mb-3">
                ...
                <div class="col-md-4">
                    <div class="form-check">
                        ...
                        <textarea for="option1" name="answer_1" id="answer_1" cols="33" rows="1" required></textarea>
                   </div>
                   <div class="form-check">
                        <input  class="form-check-input" type="radio" name="right_answer" id="option2" required value="answer_2" >
                        <textarea for="option2" name="answer_2" id="answer_2" cols="33" rows="1" required></textarea>
                   </div>
                    <div class="form-check">
                        <input  class="form-check-input" type="radio" name="right_answer" id="option3" required value="answer_3" >
                        <textarea for="option3" name="answer_3" id="answer_3" cols="33" rows="1" required></textarea>
                   </div>
